...
def predict_using_hvo(trained_model, input_tensor, voice_thresholds, voice_max_count_allowed,
...
def predict_using_h_v_o(trained_model, input_h, input_v, input_o, voice_thresholds, voice_max_count_allowed,
                      return_concatenated=False, sampling_mode=0):
    """ This method can be used to sample from a trained **MonotonicGrooveTransformer** model.
    The difference between this method and the embedded predict method is that this method allows for
    using a list of thresholds and also allows for setting the maximum number if hits allowed per voice.
    ...
    input_tensor[:, :, 2] = torch.tensor(input_h, dtype=torch.float32)
    input_tensor[:, :, 11] = torch.tensor(input_v, dtype=torch.float32)
    input_tensor[:, :, 20] = torch.tensor(input_o, dtype=torch.float32)
    trained_model.eval()
    with torch.no_grad():
...
def strip_note_from_hvo(h, v, o, drum_map):
    """ This method can be used to strip the note values from the h,v,o tensors.
    :param h:
    :param v:
    :param o:
    :return:
    ...
    voice_tags = list(drum_map.keys())
    voice_pitches = list(drum_map.values())
